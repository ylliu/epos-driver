...
int OpenDevice(unsigned int* p_pErrorCode)
{
	...
	if(0==VCS_SetGatewaySettings(g_pKeyHandle_sub,baud_rate,p_pErrorCode))
	{
		//std::cout<<"set baud rate failed"<<"Error:"<<*p_pErrorCode<<std::endl;
		LogError("VCS_SetGatewaySettings", lResult, *p_pErrorCode);
		lResult = MMC_FAILED ;	
		return false;
	}
	std::cout<<"setway baud rate to :"<<baud_rate<<std::endl;
	if(0==VCS_GetGatewaySettings(g_pKeyHandle_sub,&baud_rate,p_pErrorCode))
	{
		std::cout<<"get baud rate failed"<<std::endl;
		lResult = MMC_FAILED ;	
		return false;
	}
	std::cout<<"getway baud rate:"<<baud_rate<<std::endl;
	*/
	delete []pDeviceName;
...
bool DemoProfileCurrentMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	int lResult = MMC_SUCCESS;
	stringstream msg;
	...
	{
		LogError("VCS_ActivateProfileCurrentMode", lResult, p_rlErrorCode);
		lResult = MMC_FAILED;
	}
	else
	{
		short set_max_speed ;
		std::cout<<"input max_speed:";
		int max_speed=0;
		std::cin>>max_speed;
		int to_write_byte = 1;
		unsigned int written_byte = 0;
		if(0==VCS_SetObject(p_DeviceHandle, p_usNodeId,0x6410,0x04, &max_speed,1,&written_byte,&p_rlErrorCode))
		{
			std::cout << "set max_speed failed." << p_usNodeId<<std::endl;
			return false;
		}
		std::cout << "set max_speed success,max_speed:"<<max_speed<<"written_byte:"<<written_byte<<std::endl;
		if (0 == VCS_SetEnableState(p_DeviceHandle, p_usNodeId, &p_rlErrorCode)){
			std::cout << "set enabale failed." << p_usNodeId<<std::endl;
			return false;
		}
		std::cout << "set enabale success."<<std::endl;
		short set_current ;
		std::cout<<"input current:";
		std::cin>>set_current;
		//if(0==VCS_SetObject(p_DeviceHandle, p_usNodeId,0x2030,0x00, &set_current,1,&written_byte,&p_rlErrorCode))
		if (0 == VCS_SetCurrentMust(p_DeviceHandle, p_usNodeId, set_current, &p_rlErrorCode))
		{
		...
		{
			std::cout << "on profile current mode, last 20 seconds,node = " << p_usNodeId<<std::endl;
			sleep(20);
			if(0==VCS_SetDisableState(p_DeviceHandle, p_usNodeId, &p_rlErrorCode)){
				std::cout << "set disabale failed." << p_usNodeId<<std::endl;		
				return false;
			}
			std::cout << "set disabale success."<<std::endl;
		}
		if(VCS_GetCurrentMust(p_DeviceHandle, p_usNodeId, &set_current, &p_rlErrorCode)==0)
		{
			std::cout<<"GetCurrentSetError"<<std::endl;
			LogError("VCS_GetCurrentMust", lResult, p_rlErrorCode);
		}
		else
		{
			std::cout << "Get set current:"<<set_current<<",node = " << p_usNodeId<<std::endl;
		}
...
int Demo(unsigned int* p_pErrorCode)
{
	...
	lResult = DemoProfileCurrentMode(g_pKeyHandle, g_usNodeId, lErrorCode);
	if (lResult != MMC_SUCCESS)
	{
		LogError("DemoProfileCurrentMode", lResult, lErrorCode);
	}
	return lResult;
}
